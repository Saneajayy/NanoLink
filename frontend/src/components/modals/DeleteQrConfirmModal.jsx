      ...
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden my-8">
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-white">Delete QR Code?</h2>
          </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-start gap-3 p-4 bg-slate-950 rounded-xl border border-slate-800">
            ...
            <div className="text-sm text-slate-300 leading-relaxed">
              Are you sure you want to delete the QR code for <span className="font-bold text-white">"{qrCode.linkId?.title || qrCode.linkId?.slug || 'this link'}"</span>?
            </div>
          </div>

          {/* Educational notice per Section 6.7 */}
          <div className="flex items-start gap-2.5 p-3.5 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-xs text-indigo-300">
            <Info className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
            <span>
              This will <span className="font-bold text-white underline">not</span> delete the underlying short link (<code className="font-mono text-white">{qrCode.linkId?.shortUrl}</code>), only the QR code styling and scan tracking.
            </span>
          </div>
