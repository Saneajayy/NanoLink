...
const EditLinkModal = ({ isOpen, onClose, link, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-150 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-white border border-neutral-200 overflow-hidden my-8 shadow-2xl rounded-xl text-neutral-900">
        <div className="p-6 border-b border-neutral-200 flex items-center justify-between bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-neutral-900">Edit Short Link</h2>
              <p className="text-xs text-neutral-500 font-light">Update destination URL and display title.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 bg-white font-light">
          ...
          {/* Read-Only Short Link Slug */}
          <div className="p-4 bg-neutral-50 border border-neutral-200 rounded-lg space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-neutral-500 uppercase tracking-wider font-mono">Short Link (Read-Only)</label>
              <span className="flex items-center gap-1 text-[11px] text-amber-600 font-semibold font-mono">
                <Lock className="w-3 h-3" />
                <span>Alias permanent</span>
              </span>
            </div>
            <div className="font-mono text-sm font-bold text-green-700 select-all truncate">{link.shortUrl}</div>
          ...
          <div className="pt-2 flex justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 bg-green-700 hover:bg-green-800 text-white font-semibold rounded-lg flex items-center gap-2 text-xs disabled:opacity-70 transition-all cursor-pointer"
            >
              {loading ? (
