...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if user had pasted a URL on the homepage before signing up (Section 4 critical flow)
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-orange-500/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="mb-4 text-slate-400 text-sm z-10">
        ...
        <Link to="/login" className="text-orange-400 hover:text-orange-300 font-semibold underline underline-offset-4 transition-colors">
          Log in
        </Link>
      </div>
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden z-10 border border-slate-100">
        ...
        <div className="p-8">
          <h1 className="text-2xl font-bold text-slate-900 mb-1">Create free account</h1>
          <p className="text-slate-600 text-sm mb-6">Get 50 short links and 2 QR codes every month.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-slate-900 placeholder-slate-400 transition-all text-sm"
              />
              {/* Helper text shown under the field on signup only per Section 4 */}
              <p className="mt-1.5 text-xs text-slate-500 flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                <span>Must be at least 8 characters long and contain a special character.</span>
              </p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 px-4 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold rounded-xl shadow-lg shadow-indigo-600/25 hover:shadow-indigo-600/40 transition-all flex items-center justify-center gap-2 text-sm disabled:opacity-70 disabled:cursor-not-allowed mt-3"
            >
              {loading ? (
